...
x_train, y_train, sw_train = load_LSTM2_monthly_baseline(train_path, 50)
x_valid, y_valid, sw_valid = load_LSTM2_monthly_baseline(valid_path, 50)
x_test, y_test, sw_test    = load_LSTM2_monthly_baseline(test_path, 50)
...
model.add(LSTM(10, return_sequences=True, kernel_initializer='glorot_normal'))
model.add(Dropout(0.5))
...
checkpointer = ModelCheckpoint(filepath=output_model_path+'bestmodel_lstm2_monthly_baseline.hdf5', verbose=1, save_best_only=True)
...
os.remove(cache_path + 'history_lstm2_monthly_baseline.hdf5')
save_history(cache_path + 'history_lstm2_monthly_baseline', History)
###############################################
...
model  = load_model(output_model_path + 'bestmodel_lstm2_monthly_baseline.hdf5')
...
train, valid = load_history(cache_path + 'history_lstm2_monthly_baseline.hdf5')
plot_model_architecture(model, output_model_path + 'model_lstm2_monthly_baseline.png')
plot_loss_curve(output_model_path, 'loss_plot_lstm2_monthly_baseline', train, valid)
plot_roc(output_model_path, 'roc_lstm2_monthly_baseline', fpr, tpr)
plot_pr(output_model_path, 'pr_lstm2_monthly_baseline', rec, pre)
###############################################
