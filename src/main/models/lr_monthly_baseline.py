...
# compile the model
adam = Adam(lr=0.005, decay=1e-3)
model.compile(loss='binary_crossentropy', optimizer=adam, metrics=['accuracy'], sample_weight_mode = "temporal")

# create some callbacks
checkpointer = ModelCheckpoint(filepath=output_model_path+'bestmodel_lr_monthly_baseline.hdf5', verbose=1, save_best_only=True)
...
os.remove(cache_path + 'history_lr_monthly_baseline.hdf5')
save_history(cache_path + 'history_lr_monthly_baseline', History)
###############################################
...
model  = load_model(output_model_path + 'bestmodel_lr_monthly_baseline.hdf5')
...
train, valid = load_history(cache_path + 'history_lr_monthly_baseline.hdf5')
plot_model_architecture(model, output_model_path + 'model_lr_monthly_baseline.png')
plot_loss_curve(output_model_path, 'loss_plot_lr_monthly_baseline', train, valid)
plot_roc(output_model_path, 'roc_lr_monthly_baseline', fpr, tpr)
plot_pr(output_model_path, 'pr_lr_monthly_baseline', rec, pre)
###############################################
